...
export default async function RisultatoPage({
  ...
  if (!role || role === 'standard') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
        ...
        <p className="text-slate-600 max-w-md">I risultati sono disponibili solo per utenti Premium o Admin.</p>
      </div>
    )
  }
  ...
  // Build labels map for history display
  const questionLabelsMap: Record<string, { label: string; shortLabel: string; lookup: Record<string, number>; labels: Record<string, string> }> = {}
  for (const sq of SCORED_QUESTIONS) {
  ...
  if (supplierIds.size > 0) {
    const { data: suppliersRows } = await supabase
      .from('suppliers')
      .select('id, name, vat_number, eori_number, address, phone, email, website, contact_person')
      .in('id', [...supplierIds])
  ...
  function resolveDisplayAnswer(
    ...
    if (q.type === 'select' && q.config?.options?.length) {
      ...
      const opt = q.config.options.find((o) => String(o.value).toLowerCase().trim() === rawNorm)
  ...
  const mitigationByQuestionId = new Map<string, { previousLabel: string; newLabel: string; date: string; comment?: string | null }[]>()
  for (const entry of mitigationHistory) {
    const meta = questionLabelsMap[entry.question_id]
    const getLabel = (raw: string | null) =>
      meta ? getLabelForRaw(meta.labels, raw) : (raw ?? '—')
    const item = {
      previousLabel: getLabel(entry.previous_answer),
      newLabel: getLabel(entry.new_answer),
      date: new Date(entry.mitigated_at).toLocaleDateString('it-IT', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
      comment: entry.comment,
    }
    if (!mitigationByQuestionId.has(entry.question_id)) mitigationByQuestionId.set(entry.question_id, [])
    mitigationByQuestionId.get(entry.question_id)!.push(item)
  }

  const sectionsForPdf: { sectionTitle: string; questions: { questionId: string; questionText: string; answerText: string; riskIndex?: number; mitigation?: { previousLabel: string; newLabel: string; date: string; comment?: string | null }[] }[] }[] = sectionsData.map(section => {
    ...
    const expanded = questions.flatMap((q) => {
      ...
      const answerText = detail ? detail.answerLabel : resolveDisplayAnswer(q, rawAnswer, answerJson, supplierDetailsMap)
      return [{
        questionId: q.id,
        questionText: q.text,
        answerText: typeof answerText === 'string' ? answerText : '—',
        riskIndex: detail?.riskIndex,
        mitigation: mitigation && mitigation.length > 0 ? mitigation : undefined,
      }]
    })
    return {
      sectionTitle: section.title,
      questions: expanded.filter((q) => q.questionText),
    }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-16">
      ...
      {/* HERO OUTCOME CARD */}
      <div className={`relative overflow-hidden rounded-2xl border-2 p-8 mb-10 shadow-md ${isAccettabile
        ...
        {/* Decorative */}
        <div className={`absolute top-0 right-0 w-56 h-56 rounded-bl-full ${isAccettabile ? 'bg-[#4a7c2e]/5' : 'bg-red-500/5'
          }`} />

        <div className="relative flex items-start gap-5">
          <div className={`flex-shrink-0 p-4 rounded-2xl shadow-lg ${isAccettabile
          ...
          <div className="flex-1 min-w-0">
            <p className={`text-xs font-bold uppercase tracking-wider mb-1 ${isAccettabile ? 'text-[#4a7c2e]' : 'text-red-600'
              }`}>
              Esito Analisi
            </p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
              ...
              <div>
                <span className="text-slate-400 text-xs uppercase tracking-wide">Specie</span>
                <p className="font-semibold text-[#3d2b1a]">{specieName}</p>
              </div>
              <div>
                <span className="text-slate-400 text-xs uppercase tracking-wide">Paese</span>
                <p className="font-semibold text-[#3d2b1a]">
                  {countryName}
                  {countryHasConflicts && (
                    <span className="ml-2 text-[10px] uppercase bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full font-bold">
                      Conflitti
                    </span>
                  )}
                </p>
        ...
        <div className="absolute top-6 right-6 hidden md:block">
          <div className={`text-center rounded-xl px-4 py-2 shadow-sm border ${isAccettabile
            ...
            <p className={`text-2xl font-black ${isAccettabile ? 'text-[#4a7c2e]' : 'text-red-600'}`}>
      ...
      {/* RISK DETAIL TABLE with failing question highlighting */}
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          ...
          {failingQuestions.length > 0 && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-50 text-red-700 text-xs font-semibold border border-red-200">
              ...
              {failingQuestions.length} {failingQuestions.length === 1 ? 'criterio critico' : 'criteri critici'}
            </span>
          )}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#fcfaf7] text-left">
                <th className="px-6 py-3 text-xs font-bold uppercase tracking-wider text-[#967635]/70">#</th>
                <th className="px-6 py-3 text-xs font-bold uppercase tracking-wider text-[#967635]/70">Criterio</th>
                <th className="px-6 py-3 text-xs font-bold uppercase tracking-wider text-[#967635]/70 text-center">Risposta</th>
                <th className="px-6 py-3 text-xs font-bold uppercase tracking-wider text-[#967635]/70 text-right">Indice</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {result.details.map((d, i) => {
                const isFailing = d.riskIndex > RISK_THRESHOLD
                const riskColor = d.riskIndex <= RISK_THRESHOLD
                ...
                return (
                  <tr
                    key={d.questionId}
                    className={`transition-colors ${isFailing
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-700 max-w-md">
                      <div className="flex items-center gap-2">
                        {isFailing && <AlertTriangle className="w-4 h-4 text-red-500 flex-shrink-0" />}
                        <p className={`font-medium leading-relaxed ${isFailing ? 'text-red-900' : ''}`}>{d.label}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`text-xs font-mono uppercase ${isFailing ? 'text-red-600 font-semibold' : 'text-slate-500'}`}>
                        {d.answerLabel || '—'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className={`inline-flex px-2.5 py-1 rounded-lg text-xs font-bold ${riskColor}`}>
              ...
              {/* Overall row */}
              <tr className={`font-bold ${isAccettabile ? 'bg-[#e8f5e2]/50' : 'bg-red-50/50'}`}>
                <td className="px-6 py-4" />
                <td className="px-6 py-4 text-[#3d2b1a] text-base" colSpan={2}>
                  Rischio complessivo di fornitura (MAX)
                </td>
                <td className="px-6 py-4 text-right">
                  <span className={`inline-flex px-3 py-1.5 rounded-lg text-sm font-black ${isAccettabile ? 'bg-[#4a7c2e] text-white' : 'bg-red-600 text-white'
                    }`}>
                    {result.overallRisk.toFixed(2)}
                  </span>
                </td>
      ...
      {/* MITIGATION BUTTON — only if risk is not acceptable */}
      {!isAccettabile && (
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-red-700/80 mt-1">
                {failingQuestions.length} {failingQuestions.length === 1 ? 'criterio non supera' : 'criteri non superano'} la soglia di rischio.
                Avvia la mitigazione per aggiornare le risposte e ridurre il livello di rischio.
              </p>
            </div>
            <Link
              href={`/timberRegulation/mitigazione?session_id=${sessionId}`}
              className="flex-shrink-0 inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all duration-300 text-sm"
            >
