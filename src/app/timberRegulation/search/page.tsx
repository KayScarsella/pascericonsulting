...
import { resolveTimberWorkflowStatesBatch } from "@/lib/timber-workflow-state"
...
export default async function SearchPage({
  ...
  let formattedData: AssessmentSessionRow[] = []
  let totalPages = 1
  ...
  if (!isVerificheTab) {
    ...
    queryCount += 2
    const [{ count: verifCount }, { data: baseSessions, error: verifError }] =
      await Promise.all([
        verifCountQuery,
    ...
    if (verifError) {
      console.error("Errore durante il fetch delle verifiche Timber:", verifError)
    }
    ...
    if (baseSessions && baseSessions.length > 0) {
      ...
      verificationRows = baseSessions.map((session) => {
        ...
        const workflowState = workflowById.get(session.id)

        return {
          id: session.id,
          created_at: session.created_at || new Date().toISOString(),
          evaluation_code: session.evaluation_code || 0,
          riskCompleted: workflowState?.step1Completed ?? false,
          evaluationCompleted: workflowState?.step2Saved ?? false,
          status: session.status || "in_progress",
          final_outcome: session.final_outcome,
          isBlocked: workflowState?.isExempt ?? false,
          resume_url:
            workflowState?.resumeUrl ??
            `/timberRegulation/evaluation?session_id=${session.id}`,
          owner_name:
            (session.profiles as { full_name?: string } | null)?.full_name ?? null,
          nomeCommerciale:
  ...
  logRoutePerf("/timberRegulation/search", {
    tab,
    queryCount,
