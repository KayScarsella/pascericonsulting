...
export async function EudrSearchData({
  searchParams,
  isAdmin,
  userId,
}: {
  ...
  const tab = normalizeEudrSearchTab(searchParams.tab as string | undefined)
  const isVerificheTab = tab === "verifiche"
  const page = parsePageParam(searchParams.page, 1)
  const vpage = parsePageParam(searchParams.vpage, 1)
  const q = parseSearchParam(searchParams.q)
  const vq = parseSearchParam(searchParams.vq)
  const sort = (searchParams.sort as string) || "created_at"
  const dir = parseSortDirParam(searchParams.dir)
  const vsort = (searchParams.vsort as string) || "created_at"
  const vdir = parseSortDirParam(searchParams.vdir)
  const esito = (searchParams.esito as string) || "all"
  const stato = (searchParams.stato as string) || "all"
  const limit = 25
